...
class BPGoalEnv(GoalEnv):
    ...
    def step(self, action):
        ...
        if isinstance(self.action_space, spaces.Discrete):
            ...
            if len(action_options) == 0:
                ...
                return self.last_state, reward, bprogram_done, {}
            else:
    ...
    def _get_obs(self):
        return OrderedDict([
            ('observation', self.state_to_gym_space()),
            ('achieved_goal', self.state_to_gym_space()),
            ('desired_goal', np.array([4,6,4,7]).astype(int))
        ])
