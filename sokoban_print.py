...
def find_adjacent_objects(location, list_2):
    return [(location, l2) for l2 in list_2 if is_adjacent(location, l2)]
...
def init_bprogram():
    ...
    bthreads_list = [player(*player_location), map_printer(map)] + [box(l) for l in range(len(box_list))] + [wall(l) for l in range(len(walls_list))]
